    ...
    case msg of
        Dialog.SetPlaylistName name ->
            let
                ...
                newModel =
                    { model | playlistInfo = newPlaylistInfo }
            in
            newModel ! []
        ...
        RemoteData.Success user ->
            case model.auth of
                Just auth ->
                    let
                        playlist =
                            NewPlaylist model.playlist.name False False model.playlist.description

                        cmds =
                            [ Cmd.map Msgs.MsgForSpotify (createPlaylist user.id playlist auth.accessToken) ]

                        newModel =
                            { model | playlistModalActive = True, playlistModalLoading = True }
                    in
                    newModel ! cmds
